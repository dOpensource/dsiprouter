...
function setDynamicScriptSettings() {
    # user configurable values (setting the current value from settings.py)
    # these may be overridden by CLI args
    export DSIP_PORT=$(getConfigAttrib 'DSIP_PORT' ${DSIP_CONFIG_FILE})
    # currently the API and UI must be hosted on same port
    export DSIP_API_PORT="$DSIP_PORT"
    export RTP_PORT_MIN=$(getConfigAttrib 'RTP_PORT_MIN' ${DSIP_CONFIG_FILE})
    export RTP_PORT_MAX=$(getConfigAttrib 'RTP_PORT_MAX' ${DSIP_CONFIG_FILE})
    export KAM_SIP_PORT=$(getConfigAttrib 'KAM_SIP_PORT' ${DSIP_CONFIG_FILE})
    export KAM_SIPS_PORT=$(getConfigAttrib 'KAM_SIPS_PORT' ${DSIP_CONFIG_FILE})
    export KAM_DMQ_PORT=$(getConfigAttrib 'KAM_DMQ_PORT' ${DSIP_CONFIG_FILE})
    export KAM_WSS_PORT=$(getConfigAttrib 'KAM_WSS_PORT' ${DSIP_CONFIG_FILE})
...
function updateKamailioConfig() {
    ...
    local DSIP_API_PORT=${DSIP_API_PORT:-$(getConfigAttrib 'DSIP_API_PORT' ${DSIP_CONFIG_FILE})}
    local DSIP_API_PROTO=${DSIP_API_PROTO:-$(getConfigAttrib 'DSIP_API_PROTO' ${DSIP_CONFIG_FILE})}
    local DSIP_API_BASEURL="${DSIP_API_PROTO}://127.0.0.1:${DSIP_API_PORT}"
...
function updateRtpengineConfig() {
    local INTERFACE=""
    local RTP_PORT_MIN=${RTP_PORT_MIN:-$(getConfigAttrib 'RTP_PORT_MIN' ${DSIP_CONFIG_FILE})}
    local RTP_PORT_MAX=${RTP_PORT_MAX:-$(getConfigAttrib 'RTP_PORT_MAX' ${DSIP_CONFIG_FILE})}
    local HOMER_ID=${HOMER_ID:-$(getConfigAttrib 'HOMER_ID' ${DSIP_CONFIG_FILE})}
    local HOMER_HEP_HOST=${HOMER_HEP_HOST:-$(getConfigAttrib 'HOMER_HEP_HOST' ${DSIP_CONFIG_FILE})}
    local HOMER_HEP_PORT=${HOMER_HEP_PORT:-$(getConfigAttrib 'HOMER_HEP_PORT' ${DSIP_CONFIG_FILE})}
...
function usageOptions() {
    ...
    printf "%-30s %s\n%-30s %s\n%-30s %s\n%-30s %s\n%-30s %s\n%-30s %s\n" \
        ...
        " " "--dsip-port=<num>|--kam-sip-port=<num>|--kam-sips-port=<num>|--kam-dmq-port=<num>|--kam-wss-port=<num>|--rtp-port-min=<num>|--rtp-port-max=<num>]"
    printf "%-30s %s\n" \
            ...
            while (( $# > 0 )); do
                ...
                case $OPT in
                    ...
                    --rtpengine-uri=*)
                        ...
                        RUN_COMMANDS+=(updateRtpengineStartup)
                        ;;
                    --dsip-port=*)
                        export DSIP_PORT=$(cut -s -d '=' -f 2- <<<"$1")
                        shift
                        # sanity check
                        if ! ipPortTest "$DSIP_PORT"; then
                            printerr 'Invalid argument to option "--dsip-port="'
                            exit 1
                        fi
                        # currently the API and UI must be hosted on same port
                        export DSIP_API_PORT="$DSIP_PORT"
                        ;;
                    --kam-sip-port=*)
                        export KAM_SIP_PORT=$(cut -s -d '=' -f 2- <<<"$1")
                        shift
                        # sanity check
                        if ! ipPortTest "$KAM_SIP_PORT"; then
                            printerr 'Invalid argument to option "--kam-sip-port="'
                            exit 1
                        fi
                        ;;
                    --kam-sips-port=*)
                        export KAM_SIPS_PORT=$(cut -s -d '=' -f 2- <<<"$1")
                        shift
                        # sanity check
                        if ! ipPortTest "$KAM_SIPS_PORT"; then
                            printerr 'Invalid argument to option "--kam-sips-port="'
                            exit 1
                        fi
                        ;;
                    --kam-dmq-port=*)
                        export KAM_DMQ_PORT=$(cut -s -d '=' -f 2- <<<"$1")
                        shift
                        # sanity check
                        if ! ipPortTest "$KAM_DMQ_PORT"; then
                            printerr 'Invalid argument to option "--kam-dmq-port="'
                            exit 1
                        fi
                        ;;
                    --kam-wss-port=*)
                        export KAM_WSS_PORT=$(cut -s -d '=' -f 2- <<<"$1")
                        shift
                        # sanity check
                        if ! ipPortTest "$KAM_WSS_PORT"; then
                            printerr 'Invalid argument to option "--kam-wss-port="'
                            exit 1
                        fi
                        ;;
                    --rtp-port-min=*)
                        export RTP_PORT_MIN=$(cut -s -d '=' -f 2- <<<"$1")
                        shift
                        # sanity check
                        if ! ipPortTest "$RTP_PORT_MIN"; then
                            printerr 'Invalid argument to option "--rtp-port-min="'
                            exit 1
                        fi
                        ;;
                    --rtp-port-max=*)
                        export RTP_PORT_MAX=$(cut -s -d '=' -f 2- <<<"$1")
                        shift
                        # sanity check
                        if ! ipPortTest "$RTP_PORT_MAX"; then
                            printerr 'Invalid argument to option "--rtp-port-max="'
                            exit 1
                        fi
                        ;;
