...
function setDynamicScriptSettings() {
    ...
    if (( ${IPV6_ENABLED} == 1 )); then
        # sanity check, is the ipv6 address routable? (checking via SSH port)
        if [[ -z "$INTERNAL_IP6" ]] || ! checkConn "$INTERNAL_IP6" '22'; then
    ...
    if [[ -z "$EXTERNAL_FQDN" ]] || ! checkConn "$EXTERNAL_FQDN" '22'; then
