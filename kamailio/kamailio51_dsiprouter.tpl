...
modparam("dispatcher", "dst_avp", "$avp(dispatcher_dst)")
modparam("dispatcher", "grp_avp", "$avp(dispatcher_grp)")
modparam("dispatcher", "cnt_avp", "$avp(dispatcher_count)")
modparam("dispatcher", "sock_avp", "$avp(dispatcher_sock)")
modparam("dispatcher", "attrs_avp", "$avp(dispatcher_attrs)")
#!endif

# ----- db_cluster params ----
# set each db connection host like below
#!ifdef WITH_DBCLUSTER
...
# cluster command mode: s == serial, r == roundrobin, p == parallel (write/only)
